...
# fluidPage(
    ...
    renderDT(confusionMat[[input$Factor1]]),
...
fluidPage(
    ...
    mainPanel(    
        ...
        width = 15,
        renderDT(DT::datatable(rocResFormat)),
              options = list(
                        pageLength = 5,
...
long <- cytoAll %>%
    ...
    left_join(metaPatients %>% select(id, Dataset, Sex, State, AgeBin), by = "id") %>%
	group_by(id) %>%
	gather(key = "var", value = "Value", 
	       -Outcome, -id, -dpi, -Dataset, -Sex, -State, -AgeBin)%>% 
...
longPlots <- function(cases, v, c){

    cases %>% filter(var == v) %>%
    ggplot(aes(x = dpi, y = Value )) +
    	...
    	theme_pubr() +
        labs(x = "DPI", y = get.pretty.units(v), title = get.pretty.name(v)
             ) +
        # ggtitle(get.pretty.factor(v)) +
    	geom_hline(aes(yintercept = Threshold), 
    	           linetype="dashed", color="black") +
    	geom_smooth(method = "loess", formula = y~x, aes_string(color=c))+
...
# gp <- longPlots(cases.plot.all, "PAI1", "Dataset") %>% #test the plot %>%
...
# 
# 
# layout_ggplotly <- function(gg, x = -0.15, y = -0.08){
#   # The 1 and 2 goes into the list that contains the options for the x and y axis labels respectively
#   gp[['x']][['layout']][['annotations']][[1]][['y']] <- x #move x-axis label down
#   gp[['x']][['layout']][['annotations']][[2]][['x']] <- y #move y-axis label left
#   # gp[['x']][['layout']][['margin']][['t']] <- 110
#   gp
# }
# 
# gp %>% layout_ggplotly
...
layout_ggplotly <- function(gp, x = -0.15, y = -0.08){
  # The 1 and 2 goes into the list that contains the options for the x and y axis labels respectively
  gp[['x']][['layout']][['annotations']][[1]][['y']] <- x #move x-axis label down
  gp[['x']][['layout']][['annotations']][[2]][['x']] <- y #move y-axis label left
  # gp[['x']][['layout']][['margin']][['t']] <- 110
...
fluidRow(
    ...
    renderPlotly( longPlots(cases.plot.all, input$Factor1, input$Color) %>% #test the plot %>%
                      ggplotly(height = 500, width = 850) %>%
        layout(
            # legend = list(orientation = "h", x = 0.25, y = -0.25),
               title = list(y = 1.75), margin = list(t = 110)) %>%
        layout_ggplotly )
)
...
<!-- cases.plot.all.PAI1 <- rbind(cases.plot.fatal %>% -->
<!--     select(id, Outcome, dpi, var, Factor, Value, Threshold, Units, Timing),  -->
<!--     survive.severe.PAI1, survive.mild.PAI1) %>% ungroup() -->
